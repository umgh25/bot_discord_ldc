...
async def vote(ctx, match_id: int = None, *, team: str = None):
    ...
    try:
        ...
        print(f"Vote enregistré pour {ctx.author.name}: Match {match_id}, Équipe {team}")  # Log de débogage
        
        await ctx.send(f"✅ Vote enregistré pour {team}")
    except Exception as e:
        print(f"Erreur lors du vote: {e}")  # Log d'erreur
        await ctx.send("❌ Une erreur s'est produite lors de l'enregistrement du vote.")
import discord
...
async def test_db(ctx):
    try:
        ...
        await ctx.send(f"✅ Test DB réussi!\nTables: {tables}\nNombre de votes: {count}")
    except Exception as e:
